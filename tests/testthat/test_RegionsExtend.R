...
test_that("makeRegion", {
    ...
    expect_equal(seq_10_reg_def@name, "IMGT_VDJ_REGIONS")
...
test_that("calcBaselineOneClone", {
    library("parallel")
    load(file.path("..", "data-tests", "ExampleDb.rda"))
    db_391 <- subset(ExampleDb,CLONE == 391)
    db_3095 <- subset(ExampleDb,CLONE == 3095)
    db_3157 <- subset(ExampleDb,CLONE == 3157)
    db_baseline_391 <- calcBaselineL(db=db_391, 
                                ...
                                nproc = 1,
                                calcStats = T)
    db_baseline_3095 <- calcBaselineL(db=db_3095, 
                                    sequenceColumn="SEQUENCE_IMGT",
                                    germlineColumn="GERMLINE_IMGT_D_MASK", 
                                    testStatistic="local",
                                    regionDefinition=IMGT_V_BY_REGIONS,
                                    ...
                                    nproc = 1,
                                    calcStats = F)
    db_baseline_3157 <- calcBaselineL(db=db_3157, 
                                     sequenceColumn="SEQUENCE_IMGT",
                                     germlineColumn="GERMLINE_IMGT_D_MASK", 
    ...
    expect_equal(db_baseline_391, calcBaselineOneClone_391)
    expect_equal(db_baseline_3095, calcBaselineOneClone_3095)
    expect_equal(db_baseline_3157, calcBaselineOneClone_3157)
})