...
model is used to calculate the distance. The default model is `m1n`, a 
single nucleotide model from Smith et al., J Immunol, 1996. Other options include 
nucleotide (`ham`) and amino acid (`aa`) Hamming distance, a 3-mer mouse model 
(`m3n`, Smith et al., J Immunol, 1996), and a 5-mer model inferred from human data 
(`hs5f`, Yaari et al., Front Immunol., 2013).


```{r, eval=TRUE, warning=FALSE}
# Use genotyped V assignments and 5-mer model
db_hs5f <- distToNearest(db, vCallColumn="V_CALL_GENOTYPED", model="hs5f", first=FALSE)
...
hist(db_hs5f$DIST_NEAREST, main='Histogram - hs5f', 
     xlab='Distance to Nearest', breaks=50)
# Zoom in to find threshold at around 5
hist(db_hs5f$DIST_NEAREST, main='Histogram - hs5f - zoom', 
     xlab='Distance to Nearest', breaks=100, xlim=c(0, 15), ylim=c(0,50))

# Generate m1n histogram
hist(dist_m1n$DIST_NEAREST, main='Histogram - m1n', 
     xlab='Distance to Nearest', breaks=25, xlim=c(0, 1))
```