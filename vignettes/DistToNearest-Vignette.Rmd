...
## Calculate nearest neighbor distances across groups rather than within a group

The arguments `fields` and `cross` are used for grouping data. 

`fields` will split the input `data.frame` in groups based on values in 
the specified fields and will treat them independently. For example, 
if the input data has multiple samples, `fields="SAMPLE"` would allow 
each sample to be analyzed separately.

In the previous examples we used a subset of the original example data. In the
following example, we will use the two availabe samples, -1h and +7d, 
...
# Generate m1n histogram
p4 <- ggplot() + theme_bw() + 
    ggtitle("Grouped distance to nearest: hs1f") + xlab("distance") +
...
Setting the argument `cross` calculates the distances across groups 
to the nearest neighbor in a different group. In the following example 
we set `cross="SAMPLE"`, meaning data will be grouped in either -1h or 
+7d. For each sequence in -1h, the distances to all sequences belonging 
to other groups, +7d in this case, will be calculated and the smallest 
value returned.
...
The histogram for -1h will now be different, because we are looking at 
the distribution of the distance between the sequences in -1h and +7d. 

```{r, eval=TRUE, warning=FALSE, fig.width=7}
# Generate m1n histogram
p5 <- ggplot() + theme_bw() + 
    ggtitle("Cross-sample distance to nearest: hs1f") + xlab("distance") +
