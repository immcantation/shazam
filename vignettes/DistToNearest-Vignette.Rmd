...
transition/transversion model (Yaari et al, Front Immunol, 2013). Other options 
include nucleotide Hamming distance (`ham`), amino acid Hamming distance (`aa`), 
single nucleotide (`m1n`) and 3-mer (`m3n`) mouse models (Smith et al, J Immunol, 1996), 
and a 5-mer model inferred from human data (`hs5f`) (Yaari et al, Front Immunol, 2013).
...
dist_hs1f <- distToNearest(db, model="hs1f", first=FALSE, normalize="length", nproc=3)

# Use genotyped V assignments and 5-mer model
dist_hs5f <- distToNearest(db, vCallColumn="V_CALL_GENOTYPED", model="hs5f", first=FALSE, normalize="none", nproc=1)
```
