...
be done using two methods, namely called `dens` and `gmm`. The `findThreshold` 
function is used to choose one of the above methods. 

The `dens` method will look for the minimum 
...
The `gmm` method (computationally less expensive) follows a Gaussian Mixture Model 
(GMM) procedure, including the Expectation Maximization (EM) algorithm, for learning 
the parameters of two univariate Gaussians which fit the bimodal distribution entries. 
...
This threshold can be then invoked for assigning Ig sequences to clonal groups.

Below are some examples applying these two methods in different scenarios.
...
threshold_ham <- output@threshold
...
output <- findThreshold(dist_s5f$DIST_NEAREST, method="dens")
threshold_s5f <- output@threshold
...
# you can plot the distToNearest hist, Gaussian Fits, and optimum 
#threshold cut all together using "plotgmmFit" function
plotgmmFit(ent=db$DIST_NEAREST, GaussData=output, x_min=0.0, x_max=1, x_seq=0.1,
                histBinwidth=0.02, title="plotgmmFit")
```

Following example uses the genotyped V assignments and 5-mer via `hh_s5f` model 
discussed previously.

```{r, eval=TRUE, warning=FALSE, fig.width=7}

# To find the Threshold cut use findThreshold-switch for "gmm" method 
output <- findThreshold(dist_s5f$DIST_NEAREST, method="gmm", cutEdge=0.9)

# you can plot the distToNearest hist, Gaussian Fits, and optimum 
#threshold cut all together using "plotgmmFit" function
x_max<-dist_s5f$DIST_NEAREST[which.max(dist_s5f$DIST_NEAREST)]
plotgmmFit(ent=dist_s5f$DIST_NEAREST, GaussData=output, x_min=0.0, x_max=x_max, x_seq=10,
                histBinwidth=1, title="plotgmmFit", gauss = FALSE)
```

We note that the shape of histogram is governed by the "histBinwidth" option, meaning that
any change in bin size will change the form of the distribution, while the `gmm` method is 
