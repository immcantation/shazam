...
fontsize: 11pt
vignette: |
  %\VignetteIndexEntry{SHM Targeting models} %\VignetteEngine{knitr::rmarkdown} %\usepackage[utf8]{inputenc}
---
...
# Create a consensus mutated sequence for each clone to avoid over-counting mutational events (optional)
db_cons = calcDBClonalConsensus(db, collapseByClone = T)

# Create substitution model using silent mutations
sub_matrix <- createSubstitutionMatrix(db_cons, model="S")
# Create mutability model using silent mutations
mut_matrix <- createMutabilityMatrix(db_cons, sub_matrix, model="S")
