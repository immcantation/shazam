...
5. Calculate a nucleotide distance matrix based on the underlying SHM 
   substitution and mutability models.
...
sub_matrix <- createSubstitutionMatrix(db, model="S")
# Create mutability model using silent mutations
mut_matrix <- createMutabilityMatrix(db, sub_matrix, model="S")
...
plotMutability(model, nucleotides="A", style="hedgehog", size=0.5)
plotMutability(model, nucleotides="C", style="bar", size=0.5)
plotMutability(model, nucleotides="G", style="bar", size=0.5)
plotMutability(model, nucleotides="T", style="bar", size=0.5)
```

Calculate targeting distance matrix
--------------------------------------------------------------------------------
In the Change-O pipeline, the `hs5f` and `m3n` cloning methods rely on inferred 
targeting models. If users wish to use a targeting model inferred from their 
