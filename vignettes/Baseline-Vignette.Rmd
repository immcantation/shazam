...
--------------------------------------------------------------------------------
The first step to calculate selection is to call `getDBClonalConsensus` to 
...
db_clone <- getDBClonalConsensus(db, regionDefinition=IMGT_V_NO_CDR3, 
...
`getDBObservedMutations` is used to calculate the number of observed mutations 
and `getDBExpectedMutations` calculates the expected frequency of mutations. 
...
db_obs <- getDBObservedMutations(db_clone, sequenceColumn="CLONAL_CONSENSUS_SEQUENCE",
                               regionDefinition=IMGT_V_NO_CDR3, nproc=3)
# Calculate expected mutations and add EXPECTED columns to db_obs
db_exp <- getDBExpectedMutations(db_obs, sequenceColumn="CLONAL_CONSENSUS_SEQUENCE",
...
```{r, eval=TRUE, warning=FALSE, results="hide"}
# Calculate selection scores using the output from getDBExpectedMutations
